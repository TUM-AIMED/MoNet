...
PATH = "/home/moritz/Data/Task10_Colon/Task10_Colon"
RES = 256
RES_Z = 16
CROP_HEIGHT = 16
...
model.compile(loss=dice_l,
              optimizer='adam', metrics=[dice_c, tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])

history = model.fit(train_generator,
                    epochs=150,
                    validation_data=(X_val[..., None], y_val[..., None]),
                    steps_per_epoch=X_train_partial.shape[0]//b_size,
                    callbacks=[tf.keras.callbacks.EarlyStopping(min_delta=0.02, patience=10, restore_best_weights=True)])


model.save_weights("./serialized/weights/fpn_weights_colon.h5")

model.evaluate(X_test[..., None], y_test[..., None])
